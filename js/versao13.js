...
function checkMove(){
    ...
    for(i = 0; i < ballsNum; i++){
        ...
        balls[i].translateX(ballsVectors[i].x);
        balls[i].translateZ(ballsVectors[i].z);
...
function checkBallBallColision(b1,b2){
    ...
    // tava +0.01
    if(((altura/2)+(altura/2))**2 + 2 >= (((c1x)-(c2x))**2 + ((c1z)-(c2z))**2)){
        var v1x = ballsVectors[b1].x;
        var v1z = ballsVectors[b1].z;
        var v2x = ballsVectors[b2].x;
        var v2z = ballsVectors[b2].z;

        var coef = ((v1x - v2x)*(c1x-c2x) + (v1z-v2z)*(c1z-c2z))/((c1x-c2x)**2 + (c1z-c2z)**2);
...
function checkBallWallColisions(b1){
    ...
    else if(balls[b1].position.x >= 80){
        //ballsVectors[b1].x = -ballsVectors[b1].x;
        //scene.remove(balls[b1]);
    }
...
function updateCameraBallTarget(){
    ...
    cameraBall.position.x = x-velX*distancia;
    cameraBall.position.y = y+10;
    cameraBall.position.z = z-velZ*distancia;
...
function createBall(x, y, z, cannonBallAngle){
    ...
    var velX = getRandomArbitrary(-2.5,-2.0);

    ballsVectors[ballsNum] = new THREE.Vector3(velX, 0, cannonBallAngle);
